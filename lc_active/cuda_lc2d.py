...
def build_stream(nextf, e):
	y, x = cuda.grid(2)
	if y < nextf.shape[0] and x < nextf.shape[1]:
...
def build_neighbor(fd_nbr):
	y, x = cuda.grid(2)
	if y < fd_nbr.shape[0] and x < fd_nbr.shape[1]:
...
def cal_W(W, U, nextf):
	y, x = cuda.grid(2)
	if y < W.shape[0] and x < W.shape[1]:
...
def cal_dQ(Q, U, H, fd_nbr, A_ldg, U_lc, L1, Gamma_rot, dij):
	y, x = cuda.grid(2)
	if y < Q.shape[0] and x < Q.shape[1]:
...
def evol_Q(Q, H, W, xi, xi1, xi2, qdt):
	y, x = cuda.grid(2)
	if y < Q.shape[0] and x < Q.shape[1]:
		W1 = cuda.local.array((2, 2), dtype=numba.float64)
		M1 = cuda.local.array((2, 2), dtype=numba.float64)
		S = cuda.local.array((2, 2), dtype=numba.float64)
		...
		for i in range(W.shape[-2]):
			for j in range(W.shape[-1]):
				W1[i, j] = xi1 * W[y, x, i, j] + xi2 * W[y, x, j, i]
...
@cuda.jit
def cal_stress(sigma_p, sigma_q, Q, U, H, fd_nbr, A_ldg, U_lc, L1, xi, zeta):
	y, x = cuda.grid(2)
	if y < sigma_q.shape[0] and x < sigma_q.shape[1]:
		M1 = cuda.local.array((2, 2), dtype=numba.float64)
		H1 = cuda.local.array((2, 2), dtype=numba.float64)
		...
		summ *= 2.
		for i in range(sigma_p.shape[-1]):
			sigma_p[y, x, i] *= 2.
...
def calc_div_stress(sigma_p, sigma_q, nextf):
	y, x = cuda.grid(2)
	if y < sigma_p.shape[0] and x < sigma_p.shape[1]:
		for j in range(sigma_q.shape[3]):
			for i in range(sigma_q.shape[2]):
				ip = nextf[y, x, 1 + (2 * j)]
				im = nextf[y, x, 2 + (2 * j)]
...
def set_F_Feq(F, U, Rho, e, weights):
	y, x = cuda.grid(2)
	if y < F.shape[0] and x < F.shape[1]:
		u2 = U[y, x, 0] * U[y, x, 1]
...
if __name__=='__main__':
	...
	H = cuda.to_device(H)
	sigma_q = cuda.to_device(np.empty((Ny, Nx, 2, 2), dtype=np.float64))
	sigma_p = cuda.to_device(np.empty((Ny, Nx, 2), dtype=np.float64))
	W = cuda.to_device(np.empty((Ny, Nx, 2, 2), dtype=np.float64))

	dij = cuda.to_device(dij)
	e_d = cuda.to_device(e.astype(np.float64))
	weights = cuda.to_device(weights)
	
	#Initialize F as Feq
	build_stream[blockspergrid, threadsperblock](nextf, e)
	build_neighbor[blockspergrid, threadsperblock](fd_nbr)	#Neighborlist in +/- x, y directions
	...
	while t_current < args.t_max:
		...
		for it in range(args.t_write):
			...
			evol_F[blockspergrid, threadsperblock](F1, F2, nextf, sigma_p, 
				args.fr, args.Temp, args.itau_f, e_d, weights)

			print('Evolved F')

			cal_W[blockerspergrid, threadsperblock](W, U, nextf)
			for i in range(args.n_evol_Q):
				cal_dQ[blockspergrid, threadsperblock](Q, U, H, fd_nbr, 
					   args.A_ldg, args.U, args.L1, args.Gamma_rot, dij)
				evol_Q[blockspergrid, threadsperblock](Q, H, W, 
			...
			cal_stress[blockspergrid, threadsperblock](sigma_p, sigma_q, Q, U, H, fd_nbr,
				args.A_ldg, args.U, args.L1, args.xi, args.zeta)
			calc_div_stress[blockspergrid, threadsperblock](sigma_p, sigma_q, nextf)
			evol_F[blockspergrid, threadsperblock](F2, F1, nextf, sigma_p, 
				args.fr, args.Temp, args.itau_f, e_d, weights)
